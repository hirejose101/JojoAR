        ...
        firebaseService?.getSocialMediaFeed { [weak self] posts, error in
            DispatchQueue.main.async {
                if let error = error {
                    ...
                    // Don't show error alert for empty feed - just show empty state
                    if error.localizedDescription.contains("User not authenticated") {
                        self?.showAlert(title: "Error", message: "Please sign in to view social feed")
                    } else {
                        // For other errors, just show empty feed
                        self?.socialMediaPosts = []
